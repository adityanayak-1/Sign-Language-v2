...
VIDEOS_PATH = os.path.join("MP_VIDEOS")
...
# ── Camera ────────────────────────────────────────────────────────────────────
def open_camera(index=1, retries=30):
    ...
    cap.release()
    raise RuntimeError(
        "Could not read a frame from the webcam after {} attempts. "
        "Check that no other application is using the camera.".format(retries)
    )
